...
/**
 * This function takes a TypeScript code string, writes it to a file, and runs it in a sandbox.
 * Once the code execution is done, the sandbox is closed and the output is returned.
 * @param code Typescript code to run
 ...
 */
export async function runTypeScript(code: string, onStdout?: (out: ProcessMessage) => void, onStderr?: (out: ProcessMessage) => void) {
  // We're using a custom sandbox that's defined in the `zapier-ts-sandbox` directory.
  console.log('= Starting sandbox (zapier-ts-sandbox)...')
  console.time('sandbox-start')
  const sbx = await Sandbox.create('zapier-ts-sandbox')
  ...
  sbx.filesystem.write('/home/user/index.mts', code)
