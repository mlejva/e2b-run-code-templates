...
/**
 * This function takes a JavaScript code string, writes it to a file, and runs it in a sandbox.
 ...
 */
export async function runJavaScript(code: string, onStdout?: (out: ProcessMessage) => void, onStderr?: (out: ProcessMessage) => void) {
  // We're using a custom sandbox that's defined in the `zapier-js-sandbox` directory.
  console.log('= Starting sandbox (zapier-js-sandbox)...')
  console.time('sandbox-start')
  const sbx = await Sandbox.create('zapier-js-sandbox')
  ...
  sbx.filesystem.write('/home/user/index.mjs', code)
